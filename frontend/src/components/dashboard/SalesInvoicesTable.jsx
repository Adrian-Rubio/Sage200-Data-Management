        ...
        <div className="p-4 flex flex-col h-full">
            <div className="flex justify-between items-center mb-6">
                <div className="flex flex-col">
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight"> Mostrando {invoices.length} de {total} resultados</p>
                </div>
                ...
                <div className="relative">
                    <input
                        ...
                        className="pl-10 pr-4 py-1.5 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 w-64 transition-all"
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                    />
                    <svg className="absolute left-3 top-2 h-3.5 w-3.5 text-slate-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="overflow-x-auto flex-grow">
                <table className="min-w-full divide-y divide-slate-100">
                    <thead className="bg-slate-50/50">
                        <tr>
                            <th className="px-4 py-3 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Nº Factura</th>
                            <th className="px-4 py-3 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Fecha</th>
                            <th className="px-4 py-3 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Cliente</th>
                            <th className="px-4 py-3 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Comercial</th>
                            <th className="px-4 py-3 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">Importe</th>
                        </tr>
                    </thead>
                    <tbody className={`bg-white divide-y divide-slate-50 ${isLoading ? 'opacity-50' : ''}`}>
                        {filteredData.length > 0 ? (
                                ...
                                <tr key={idx} className="hover:bg-slate-50/80 transition-colors group">
                                    ...
                                    <td className="px-4 py-2.5 whitespace-nowrap text-xs text-slate-700">
                                        <div className="font-semibold max-w-[300px] truncate flex items-center gap-2" title={inv.Cliente}>
                                            <span className="text-slate-300 font-black text-[10px]">{inv.CodigoCliente}</span>
                                            {inv.Cliente}
                                        </div>
                                    </td>
                                    <td className="px-4 py-2.5 whitespace-nowrap">
                                        <span className="bg-slate-50 text-slate-500 border border-slate-100 px-2 py-0.5 rounded-[4px] text-[9px] font-black uppercase tracking-tighter">
                                            {inv.Comisionista}
                                        </span>
                                    </td>
                                    <td className="px-4 py-2.5 whitespace-nowrap text-xs text-right font-black text-slate-900">
                            ...
                            <tr>
                                <td colSpan="5" className="px-6 py-10 text-center text-sm text-slate-400 font-bold italic">
            ...
            <div className="flex justify-between items-center mt-6 pt-4 border-t border-slate-50">
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-bold hover:bg-blue-700 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm"
                    >
                        Siguiente
                    </button>
                </div>
