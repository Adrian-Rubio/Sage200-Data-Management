    ...
    return (
        <div className="animate-fadeIn pb-10">
            ...
            <div className="flex items-center justify-between mb-8">
                <button onClick={onBack} className="flex items-center gap-2 text-slate-400 hover:text-blue-600 font-bold text-xs uppercase transition-colors group">
                ...
                <div className="text-right">
                    <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Ficha de Artículo</span>
                    <h1 className="text-3xl font-black text-slate-800 tracking-tighter">{articleCode}</h1>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div className="lg:col-span-2 bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 flex flex-col justify-between relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-8 opacity-5">
                        <svg className="w-32 h-32" fill="currentColor" viewBox="0 0 24 24"><path d="M13 9h8L11 24v-9H3l10-15v9z" /></svg>
                    </div>
                    <div>
                        <h2 className="text-2xl font-black text-slate-800 mb-2 leading-tight">{info?.description}</h2>
                        <p className="text-slate-400 font-medium mb-6 uppercase text-xs tracking-wider">{info?.description2 || 'Sin descripción adicional'}</p>

                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                            <div>
                                <span className="text-[10px] font-black text-slate-400 uppercase block mb-1">Precio Venta</span>
                                <span className="text-lg font-bold text-slate-700">{info?.sale_price ? new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(info.sale_price) : '---'}</span>
                            </div>
                            <div>
                                <span className="text-[10px] font-black text-slate-400 uppercase block mb-1">Últ. Coste</span>
                                <span className="text-lg font-bold text-slate-700">{info?.purchase_price ? new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(info.purchase_price) : '---'}</span>
                            </div>
                            <div>
                                <span className="text-[10px] font-black text-slate-400 uppercase block mb-1">U. Medida</span>
                                <span className="text-lg font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg">{info?.unit || 'UN'}</span>
                            </div>
                            <div>
                ...
                {/* Stock Card */}
                <div className="bg-blue-600 rounded-[2rem] p-8 shadow-xl text-white flex flex-col justify-between relative overflow-hidden group">
                    <div className="absolute -bottom-6 -right-6 opacity-10 group-hover:scale-110 transition-transform duration-700">
                        <svg className="w-48 h-48" fill="currentColor" viewBox="0 0 24 24"><path d="M20 7h-4V5l-2-2h-4L8 5v2H4c-1.1 0-2 .9-2 2v11c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V9c0-1.1-.9-2-2-2z" /></svg>
                    </div>
                    <div className="z-10">
