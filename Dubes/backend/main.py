...
from sqlalchemy import func, text, Date, Integer
...
app = FastAPI(title="Misstipsi Dashboard API (Cached)")
...
# Función para filtrar fechas y local de forma segura en SQLite
def filter_date_range(query, start, end, local_id: Optional[str] = None):
    # Usamos func.datetime para normalizar la comparación en SQLite
    query = query.filter(
...
@app.get("/")
def read_root():
    return {"message": "Gulah Dashboard API (Cached Mode) is running"}
...
@app.get("/api/hours/distribution")
...
def get_recent_tickets(
    ...
    s_bound, e_bound = parse_date_filter(start_date, end_date)
    # Usamos joinedload para traer relaciones y evitar consultas N+1
    ...
    for s in sales:
        ...
        # Calcular duración de la mesa (en minutos)
        duration = 0
...
@app.get("/api/invitations/details")
